  ...
  return (
    <html lang="en">
      <body className={`${sans.variable} ${serif.variable} ${mono.variable} font-sans min-h-screen flex flex-col`}>
        <a
          href="#main-content"
          className="sr-only z-50 rounded bg-background px-3 py-2 text-sm font-semibold focus:not-sr-only focus:fixed focus:left-3 focus:top-3"
        >
        ...
        <header className="no-print sticky top-0 z-40 border-b bg-background/90 backdrop-blur">
          <div className="container flex h-14 items-center justify-between">
            ...
            <nav aria-label="Primary navigation" className="flex items-center gap-1 text-sm">
              ...
              <Link href="/document-check" className="rounded-md px-3 py-1.5 font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground">
                Document check
              </Link>
        ...
        <footer className="no-print border-t py-8">
          <div className="container flex flex-col items-center justify-between gap-3 text-xs text-muted-foreground sm:flex-row">
            ...
            <p>Deterministic engine · Zero LLM in the computation path · <Link href="/privacy" className="underline underline-offset-2">Privacy &amp; retention</Link></p>
          </div>
        </footer>
      </body>
    </html>
